...
bool old2New( const char* pathToOldMap ) {
  ...
  for( pugi::xml_node::iterator rowIter = doc.first_child().first_child().begin(); rowIter != doc.first_child().first_child().end(); rowIter++ ) {
    if( !strcmp( ( *rowIter ).name(), "EnemyEntity" ) ) {
      entities.push_back( EntityStruct{ (uint16_t)0,
  ...
  for( pugi::xml_node::iterator rowIter = doc.first_child().begin(); rowIter != doc.first_child().end(); rowIter++ ) {
    ...
    for( pugi::xml_node::iterator tileIter = ( *rowIter ).begin(); tileIter != ( *rowIter ).end(); tileIter++ ) {
      if( !strcmp( ( *tileIter ).name(), "EmptyTile" ) ) {
        ...
        rows[y].push_back( TileStruct{ (uint16_t)1,
                                       ( uint16_t )( *tileIter ).attribute( "tileType" ).as_uint(),
                                       ( uint32_t )( *tileIter ).attribute( "tileColor" ).as_uint(),
                                       (int)1,
                                       ( uint16_t )( *tileIter ).attribute( "xTo" ).as_uint(),
                                       ( uint16_t )( *tileIter ).attribute( "yTo" ).as_uint(),
                                       ( uint16_t )( *tileIter ).attribute( "levelIndexTo" ).as_uint() } );
      } else if( !strcmp( ( *tileIter ).name(), "WallTile" ) ) {
        rows[y].push_back( TileStruct{ (uint16_t)2,
                                       ( uint16_t )( *tileIter ).attribute( "tileType" ).as_uint(),
                                       ( uint32_t )( *tileIter ).attribute( "tileColor" ).as_uint(),
                                       (int)( *tileIter ).attribute( "tileTerrainType" ).as_int(),
                                       (uint16_t)0,
                                       (uint16_t)0,
                                       (uint16_t)0 } );
      } else if( !strcmp( ( *tileIter ).name(), "WaterTile" ) ) {
        rows[y].push_back( TileStruct{ (uint16_t)3,
                                       ( uint16_t )( *tileIter ).attribute( "tileType" ).as_uint(),
                                       ( uint32_t )( *tileIter ).attribute( "tileColor" ).as_uint(),
                                       (int)2,
                                       (uint16_t)0,
