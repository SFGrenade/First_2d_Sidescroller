...
BaseEntity::BaseEntity(double x, double y, uint16_t sizeX, uint16_t sizeY) :
		_position( { (double) (x * gridWidth), (double) (y * gridWidth), (uint16_t) (sizeX * gridWidth), (uint16_t) (sizeY * gridWidth) }), up(false), down(false), left(false), right(false), colUp(false), colDown(true), colLeft(false), colRight(false), moveXSpeed(100.0), moveYSpeed(0.0), tilemap(NULL), level(NULL) {
}
...
void BaseEntity::setPos(double x, double y) {
	_position = *(new OwnRectEntity { x, y, _position.w, _position.h });
}
