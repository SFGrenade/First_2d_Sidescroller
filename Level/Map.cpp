...
Map::Map(BGMPlayer* newBgmPlayer) :
		selectedLevel(NULL), bgmPlayer(newBgmPlayer), currentIndex(-1) {
}
		...
		for (pugi::xml_node::iterator rowIter = doc.first_child().begin(); rowIter != doc.first_child().end(); rowIter++) {
			...
			for (pugi::xml_node::iterator tileIter = (*rowIter).begin(); tileIter != (*rowIter).end(); tileIter++) {
				if (!strcmp((*tileIter).name(), "EmptyTile")) {
					...
					tmpLevel->setTile(x, y, new game::WallTile((*tileIter).attribute("tileType").as_uint(), (*tileIter).attribute("tileColor").as_uint(), (*tileIter).attribute("tileTerrainType").as_int()));
					...
					tmpLevel->setTile(x, y, new game::TransitionTile((*tileIter).attribute("xTo").as_uint(), (*tileIter).attribute("yTo").as_uint(), (*tileIter).attribute("levelIndexTo").as_uint(), (*tileIter).attribute("tileType").as_uint(), (*tileIter).attribute("tileColor").as_uint()));
				}
				x++;
			}
			y--;
...
Level* Map::loadLevelNew(uint16_t levelNum, BGMPlayer* bgmPlayer) {
	...
	if (!strcmp(sflvlCompare, sflvltext)) {
		return NULL;
	}
