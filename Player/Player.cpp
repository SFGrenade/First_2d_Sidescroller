...
Player::Player( double x, double y, uint16_t sizeX, uint16_t sizeY )
    : _position( { (double)( x * gridWidth ), (double)( y * gridWidth ), (uint16_t)sizeX, (uint16_t)sizeY } ),
      up( false ),
      down( false ),
      left( false ),
      right( false ),
...
void Player::updatePosition( uint64_t msSinceLastFrame, Map* map ) {
  ...
  if( left ) {
    ...
    for( uint32_t i = 0; i < (uint16_t)_position.h; i++ ) {
      ...
      if( ( ( *tilemap )[yCheck][newXPos / gridWidth] ) && ( ( *tilemap )[yCheck][newXPos / gridWidth]->getType() != 0 ) ) {
        if( !contains( &collisions, ( *tilemap )[yCheck][newXPos / gridWidth] ) ) {
          collisions.push_back( ( *tilemap )[yCheck][newXPos / gridWidth] );
          xCollisions++;
        }
      }
    }
  ...
  if( this->right ) {
    ...
    for( uint32_t i = 0; i < (uint16_t)_position.h; i++ ) {
      uint16_t yCheck = ( _position.y + i ) / gridWidth;
      if( ( ( *tilemap )[yCheck][( newXPos + _position.w ) / gridWidth] ) && ( ( *tilemap )[yCheck][( newXPos + _position.w ) / gridWidth]->getType() != 0 ) ) {
        if( !contains( &collisions, ( *tilemap )[yCheck][( newXPos + _position.w ) / gridWidth] ) ) {
          collisions.push_back( ( *tilemap )[yCheck][( newXPos + _position.w ) / gridWidth] );
          xCollisions++;
        }
      }
    }
  ...
  if( moveYSpeed > 0 ) {
    ...
    for( uint32_t i = 0; i < (uint16_t)_position.w; i++ ) {
      ...
      if( ( ( *tilemap )[( newYPos + _position.h ) / gridWidth][xCheck] ) && ( ( *tilemap )[( newYPos + _position.h ) / gridWidth][xCheck]->getType() != 0 ) ) {
        if( !contains( &collisions, ( *tilemap )[( newYPos + _position.h ) / gridWidth][xCheck] ) ) {
          collisions.push_back( ( *tilemap )[( newYPos + _position.h ) / gridWidth][xCheck] );
          yCollisions++;
        }
      }
    }
    ...
    for( uint32_t i = 0; i < (uint16_t)_position.w; i++ ) {
      uint16_t xCheck = ( _position.x + i ) / gridWidth;
      if( ( ( *tilemap )[newYPos / gridWidth][xCheck] ) && ( ( *tilemap )[newYPos / gridWidth][xCheck]->getType() != 0 ) ) {
        if( !contains( &collisions, ( *tilemap )[newYPos / gridWidth][xCheck] ) ) {
          collisions.push_back( ( *tilemap )[newYPos / gridWidth][xCheck] );
          yCollisions++;
        }
      }
    }
