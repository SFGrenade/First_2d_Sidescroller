...
class RevolverList {
  ...
  public:
  RevolverList() : length( 0 ), first( NULL ), last( NULL ), current( NULL ) {}
  ...
  void rollNext() { current = current->getNext(); }
  void rollPrev() { current = current->getPrev(); }
  ...
  RevolverListItem< _TYPE >* getFirst() { return first; }
  RevolverListItem< _TYPE >* getLast() { return last; }
  ...
  _TYPE getCurrentData( int offset = 0 ) { return getCurrent( offset )->getData(); }
